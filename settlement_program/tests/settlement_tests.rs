...
/// (REAL generation bump via key rotation) -> second store-only transition at
/// the new generation -> buffer close with rent refund.
#[tokio::test]
async fn test_settle_full_flow() -> TestResult<()> {
    let mut fixture = SettlementTestBuilder::new().await;
    let mut test_ncn = fixture
        .create_initial_test_ncn(OPERATOR_COUNT, None)
        .await?;
    let ncn = test_ncn.ncn_root.ncn_pubkey;
    let generation = fixture.current_generation(&ncn).await?;
    // Phase 1: every operator registration bumps the generation once.
    assert_eq!(generation, OPERATOR_COUNT as u64);
    ...
    let args = settle_args(&fixture, &test_ncn, payload, &[], generation);
    ...
    // ---- Wrong-generation certificate: a REAL generation bump ----
    // Assemble a full-quorum certificate against the current generation, then
    // rotate operator 0's BLS keys before it lands. The rotation bumps the
    // snapshot generation, so the in-flight certificate dies at the
    // generation gate (before any pairing runs).
    let payload = store_payload(&state, 1, [0xCD; 32]);
    let in_flight_args = settle_args(&fixture, &test_ncn, payload, &[], generation);
    fixture.rotate_operator_keys(&mut test_ncn, 0).await?;
    let new_generation = fixture.current_generation(&ncn).await?;
    assert_eq!(new_generation, generation + 1);
    let result = settlement
        .do_settle(&ncn, &state, None, &in_flight_args)
        .await;
    assert_ncn_program_error(result, NCNProgramError::SnapshotGenerationMismatch, Some(1));

    // ---- Second transition: store-only payload re-assembled at the new
    // generation (operator 0 signs with its rotated key) ----
    let payload = store_payload(&state, 1, [0xCD; 32]);
    let args = settle_args(&fixture, &test_ncn, payload, &[], new_generation);
...
async fn test_settle_certificate_and_payload_gates() -> TestResult<()> {
    ...
    // ---- 2 of 4 signers = 5000 bps < 6667 (Config.consensus_threshold_bps
    // default): InsufficientStakeBps ----
    let payload = store_payload(&state, 0, [0x11; 32]);
    let args = settle_args(&fixture, &test_ncn, payload, &[2, 3], generation);
    ...
    let payload = store_payload(&state, 0, [0x11; 32]);
    let args = settle_args(&fixture, &test_ncn, payload, &[3], generation);
    settlement.do_settle(&ncn, &state, None, &args).await?;
    ...
    let mut args = settle_args(&fixture, &test_ncn, payload, &[], generation);
    ...
    let mut args = settle_args(&fixture, &test_ncn, signed_payload, &[], generation);
