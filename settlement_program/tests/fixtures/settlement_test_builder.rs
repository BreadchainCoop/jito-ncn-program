...
impl SettlementTestBuilder {
    ...
    /// Reads the snapshot's current operator-set generation (bumped by
    /// register / remove / key-rotation, Phase 1 semantics).
    pub async fn current_generation(&mut self, ncn: &Pubkey) -> TestResult<u64> {
        let mut ncn_program_client = self.ncn_program_client();
        Ok(ncn_program_client.get_snapshot(*ncn).await?.generation())
    }

    /// Rotates operator `index`'s BLS keys through the REAL
    /// `UpdateOperatorBn128Keys` instruction — bumping the snapshot
    /// generation — and updates the local `OperatorRoot` so subsequent
    /// [`Self::sign_digest`] calls sign with the NEW key.
    pub async fn rotate_operator_keys(
        &mut self,
        test_ncn: &mut TestNcn,
        index: usize,
    ) -> TestResult<()> {
        let mut ncn_program_client = self.ncn_program_client();
        let ncn = test_ncn.ncn_root.ncn_pubkey;
        let operator_root = &mut test_ncn.operators[index];

        let new_privkey = PrivKey::from_random();
        let new_g1_pubkey = G1Point::try_from(new_privkey).unwrap();
        let new_g1_compressed = G1CompressedPoint::try_from(new_g1_pubkey).unwrap();
        let new_g2_pubkey = G2Point::try_from(&new_privkey).unwrap();
        let new_g2_compressed = G2CompressedPoint::try_from(&new_privkey).unwrap();
        let pop_signature = new_privkey
            .sign::<Sha256Normalized>(&pop_message_digest(
                &ncn,
                &operator_root.operator_pubkey,
                &new_g1_compressed.0,
        ...
        operator_root.bn128_privkey = new_privkey;
        operator_root.bn128_g1_pubkey = new_g1_pubkey;
        operator_root.bn128_g2_pubkey = new_g2_pubkey;
        Ok(())
    }
